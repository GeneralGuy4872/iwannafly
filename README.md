...
Note: Motorola Notation is used in game and in documentation, meaning that $n is a hexadecimal number and @n is an octal number. latitude and longitude are recorded as d\*m'. the symbol for currency shall be an uppercase, hi-lited "G" (rendered using <CODE>\x1b7mG\x1bm</CODE> on an ansi-compliant terminal emulator).
