struct bone *sculpt_avian(geomet)
  ...
  //{*up,*prev,*next,{off.x,off.y,off.z},{len.x,len.y,len.z},matrix base,matrix curr,bool drawline,shape *geom}
    ...
    *(pelvis->geom) = shape_poly_triangle(SUITFIT(geomet,coxyx),SUITFIT(geomet,hip)/2)
  ...
  struct bone *neck = spine(index_spine,index_spine,SUITFIT(geomet,neck),7)
    ...
    *(skull->geom) = shape_poly_octo(SUITFIT(geomet,skull))
    doublelink(skull)
    struct bone *lshoulder = malloc(sizeof(struct bone))
    *lshoulder = {index_spine,skull,NULL,{FR_ZERO,FR_ZERO,FR_ZERO},{SUITFIT(geomet,shld),0,0},matgen_z_deg(LEFT * 90),matgen_ident,TRUE,NULL}
  ...
  struct bone *rleg = digiti(pelvis,ltoe1,SUITFIT(geomet,femr),(SUITFIT(geomet,q_angle),15),RIGHT)
  ...
  //nmax is the length of the array (meaning nmax-1 is the last element)
    ...
    *(tail->geom) = shape_poly_xfan(SUITFIT(geomet,tail),SUITFIT(geomet,fan_out)/2,4)
    doublelink(tail)
  return pelvis
  }
